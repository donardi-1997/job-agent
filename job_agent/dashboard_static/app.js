...
function decisionLabel(band) {
	return { ignore: 'Ignorar', save: 'Guardar', recommend: 'Recomendada', prepare: 'Postular' }[band] || band || '—';
}
...
function renderJobs(jobs) {
	...
	body.querySelectorAll('[data-review-job]').forEach((button) => {
		button.addEventListener('click', () => openJob(Number(button.dataset.reviewJob)));
	});
}

function chips(items, emptyText) {
	return items?.length
		? items.map((item) => `<span class="skill-chip">${escapeHtml(item)}</span>`).join('')
		: `<span class="empty-chip">${escapeHtml(emptyText)}</span>`;
}

function addFrequentAnswerRow(item = { question: '', answer: '' }) {
	const row = document.createElement('div');
	row.className = 'frequent-answer-row';
	...
	$('#frequentAnswers').appendChild(row);
}
...
function renderProfile(profile) {
	...
	$('#salaryInput').value = profile.salary_expectation || '';
	$('#availabilityInput').value = profile.availability || '';
	$('#workModesInput').value = (profile.preferred_work_modes || []).join(', ');
	$('#contractsInput').value = (profile.preferred_contract_types || []).join(', ');
	$('#frequentAnswers').innerHTML = '';
	(profile.frequent_answers || []).forEach(addFrequentAnswerRow);
	if (!(profile.frequent_answers || []).length) addFrequentAnswerRow();

	if (profile.target_roles?.length && !$('#keywordInput').dataset.userEdited) {
		$('#keywordInput').value = profile.target_roles[0];
	}
	if (profile.preferred_locations?.length) {
		if (!$('#locationInput').dataset.userEdited) $('#locationInput').value = profile.preferred_locations[0];
		if (!$('#batchLocationInput').dataset.userEdited) $('#batchLocationInput').value = profile.preferred_locations[0];
	}
	$('#batchScoreInput').value = profile.prepare_application_score ?? 85;
	// Deliberately do not populate batchKeywordInput: all profile roles are already searched automatically.
	$('#batchKeywordInput').value = '';
}

function collectFrequentAnswers() {
	return [...document.querySelectorAll('.frequent-answer-row')]
		.map((row) => ({
...
async function saveProfile(event) {
	...
	const payload = {
		...
		preferred_work_modes: splitCsv($('#workModesInput').value),
		preferred_contract_types: splitCsv($('#contractsInput').value),
		frequent_answers: collectFrequentAnswers(),
	};
	try {
		...
		$('#profileMessage').textContent = `Perfil actualizado. El autopiloto usará ${profile.target_roles?.length || 0} cargos objetivo.`;
	} catch (error) {
...
function renderBatchStatus(status) {
	...
	const running = ['searching', 'applying'].includes(status.state);
	box.dataset.state = running ? 'running' : status.state || 'idle';
	button.disabled = running;
	button.textContent = status.state === 'searching' ? 'Buscando mis cargos…' : status.state === 'applying' ? 'Postulando…' : 'Ejecutar mi lote';
	const titles = {
		idle: 'Autopiloto listo',
		searching: 'Buscando en mis cargos',
		applying: 'Auto-postulación en curso',
		completed: 'Lote personal finalizado',
		error: 'El lote necesita atención',
	};
	$('#batchStatusTitle').textContent = titles[status.state] || 'Estado del lote';
	let detail = status.message || 'Usará los cargos objetivo guardados en Mi perfil.';
	if (status.current_search_term) {
		const total = status.search_terms?.length || 0;
		const done = status.searches_completed ?? 0;
		detail = `${detail} · ${status.current_search_term}${total ? ` (${Math.min(done + 1, total)}/${total})` : ''}`;
	} else if (status.current_job_title) {
		detail = `${detail} · ${status.current_job_title}`;
	}
	$('#batchStatusMessage').textContent = detail;
	$('#batchFound').textContent = status.found ?? 0;
	$('#batchEligible').textContent = status.eligible ?? 0;
	$('#batchAttempted').textContent = status.attempted ?? 0;
	$('#batchSubmitted').textContent = status.submitted ?? 0;
	$('#batchBlocked').textContent = status.blocked ?? 0;
}

function renderBatchHistory(items) {
	const root = $('#batchHistory');
	if (!items?.length) {
		root.innerHTML = '<span class="empty-chip">Sin ejecuciones todavía.</span>';
		return;
	}
	root.innerHTML = items.map((run) => `<article class="batch-history-row"><div><strong>${escapeHtml(run.keyword)}</strong><span>${escapeHtml(run.location)} · score ≥ ${run.min_score}</span></div><div class="batch-history-stats"><span>${run.found} únicas</span><span>${run.submitted}/${run.attempted} enviadas</span><span class="batch-state batch-${escapeHtml(run.state)}">${escapeHtml(run.state)}</span></div></article>`).join('');
}

async function loadBatchHistory() {
	try {
		renderBatchHistory(await fetchJson('/api/batch/history'));
	} catch (error) {
		console.error('Unable to load batch history', error);
	}
}

async function pollBatchStatus() {
	try {
		const status = await fetchJson('/api/batch/status');
		...
		console.error('Unable to poll batch status', error);
	}
}

async function startBatch(event) {
	event.preventDefault();
	if (!loadedProfile?.target_roles?.length) {
		renderBatchStatus({ state: 'error', message: 'Configura al menos un cargo objetivo en Mi perfil.' });
		return;
	}
	const payload = {
		keyword: $('#batchKeywordInput').value.trim(),
		location: $('#batchLocationInput').value.trim(),
		max_results: Number($('#batchResultsInput').value),
	...
	try {
		...
		renderBatchStatus(status);
		if (batchPoll) clearInterval(batchPoll);
		batchPoll = setInterval(pollBatchStatus, 1500);
	} catch (error) {
		renderBatchStatus({ state: 'error', message: error.message });
	}
}
...
function renderDraft(draft) {
	...
	const outcome = draft.submitted
		? '<span class="answer-state">Postulación enviada</span>'
		: draft.blocked_reason
			? `<span class="draft-warning">${escapeHtml(draft.blocked_reason)}</span>`
			: '';
	...
	$('#draftQuestions').innerHTML = questions.length
		? questions.map((item, index) => {
			const options = item.options?.length ? `<div class="draft-options">Opciones: ${item.options.map(escapeHtml).join(' · ')}</div>` : '';
			return `<article class="draft-question ${item.requires_user_input ? 'needs-input' : 'ready'}" data-draft-index="${index}"><div class="question-top"><span>#${index + 1}</span><strong>${escapeHtml(item.question)}</strong><em>${escapeHtml(item.confidence)}%</em></div>${options}<label class="draft-edit-label">Respuesta utilizada<input class="draft-answer-input" value="${escapeHtml(item.suggested_answer || '')}" placeholder="Respuesta"></label><label class="draft-review-toggle"><input class="draft-needs-input" type="checkbox" ${item.requires_user_input ? 'checked' : ''}> Requiere información adicional</label>${item.note ? `<p class="draft-note">${escapeHtml(item.note)}</p>` : ''}</article>`;
		}).join('')
		: '<p class="empty-chip">No se registraron preguntas en este intento.</p>';
	$('#draftMessage').textContent = draft.submitted
		? 'Las respuestas usadas en la postulación quedaron guardadas localmente.'
		: 'Puedes corregir y guardar estas respuestas para un próximo intento.';
}

function collectDraft() {
	if (!activeDraft) return null;
	const questions = (activeDraft.questions || []).map((item, index) => {
		const card = document.querySelector(`[data-draft-index="${index}"]`);
		return {
			...item,
			suggested_answer: card?.querySelector('.draft-answer-input')?.value.trim() || '',
			requires_user_input: Boolean(card?.querySelector('.draft-needs-input')?.checked),
		};
	});
...
async function saveDraft() {
	if (!activeJobId || !activeDraft) return;
	try {
		const saved = await fetchJson(`/api/jobs/${activeJobId}/draft`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(collectDraft()),
		});
		renderDraft(saved);
		$('#draftMessage').textContent = 'Respuestas guardadas localmente.';
	} catch (error) {
		$('#draftMessage').textContent = `No se pudo guardar: ${error.message}`;
	}
}

function setPrepareStatus(status) {
	const box = $('#prepareStatus');
	const button = $('#prepareButton');
	box.dataset.state = status.state || 'idle';
	const running = status.state === 'running';
	button.disabled = running;
	button.textContent = running && status.job_id === activeJobId ? 'Postulando…' : 'Postular automáticamente';
	const titles = {
		idle: 'Sin intento',
		running: 'Postulación en ejecución',
		completed: status.submitted ? 'Postulación enviada' : 'Intento finalizado',
		error: 'La postulación necesita atención',
	};
	$('#prepareStatusTitle').textContent = titles[status.state] || 'Estado de postulación';
	$('#prepareStatusMessage').textContent = status.message || 'Puedes postularte automáticamente cuando quieras.';
}
	...
	try {
		...
		if (activeJobId === jobId) {
			...
			setPrepareStatus({
				...
				message: draft.submitted ? 'Postulación enviada y respuestas guardadas.' : (draft.blocked_reason || 'Respuestas guardadas del último intento.'),
			});
		}
	} catch (_) {
		if (activeJobId === jobId) {
			...
			setPrepareStatus({ state: 'idle', job_id: jobId });
		}
	}
}
...
async function openJob(jobId) {
	try {
		...
		$('#drawerDecision').textContent = job.status === 'applied' ? 'Postulada' : decisionLabel(job.band);
...
async function updateJobStatus(status) {
	...
	await fetchJson(`/api/jobs/${activeJobId}/status`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
...
async function pollPrepareStatus() {
	try {
		...
		if (activeJobId === status.job_id) setPrepareStatus(status);
		if (status.state !== 'running') {
			if (preparePoll) clearInterval(preparePoll);
			preparePoll = null;
			if (status.job_id) await loadDraft(status.job_id);
			await loadDashboard();
		}
	} catch (error) {
		console.error(error);
...
async function prepareApplication() {
	...
	try {
		const status = await fetchJson(`/api/jobs/${jobId}/prepare`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: '{}',
		});
		...
		setPrepareStatus({ state: 'error', job_id: jobId, message: error.message });
	}
}
...
function renderSearchStatus(status) {
	...
	$('#searchStatusMessage').textContent = status.message || 'Configura una búsqueda puntual cuando la necesites.';
}
...
async function loadProfile() {
	try {
		renderProfile(await fetchJson('/api/profile'));
	} catch (error) {
		$('#profileMessage').textContent = error.message;
	}
}
...
async function startSearch(event) {
	...
	try {
		const state = await fetchJson('/api/search', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(payload),
		});
...
$('#keywordInput').addEventListener('input', () => { $('#keywordInput').dataset.userEdited = '1'; });
$('#locationInput').addEventListener('input', () => { $('#locationInput').dataset.userEdited = '1'; });
$('#batchLocationInput').addEventListener('input', () => { $('#batchLocationInput').dataset.userEdited = '1'; });
$('#refreshButton').addEventListener('click', () => Promise.all([loadDashboard(), loadProfile(), loadBatchHistory()]));
$('#focusSearchButton').addEventListener('click', () => { $('#search').scrollIntoView({ behavior: 'smooth' }); $('#keywordInput').focus(); });
$('#searchForm').addEventListener('submit', startSearch);
$('#batchForm').addEventListener('submit', startBatch);
$('#profileForm').addEventListener('submit', saveProfile);
$('#addFrequentAnswer').addEventListener('click', () => addFrequentAnswerRow());
...
Promise.all([loadDashboard(), loadProfile(), loadBatchHistory(), loadApplicationMode()]).then(async () => {
	...
	if (prep?.state === 'running') preparePoll = setInterval(pollPrepareStatus, 1500);
	const batch = await fetchJson('/api/batch/status').catch(() => null);
	if (batch) renderBatchStatus(batch);
	if (batch && ['searching', 'applying'].includes(batch.state)) batchPoll = setInterval(pollBatchStatus, 1500);
});
