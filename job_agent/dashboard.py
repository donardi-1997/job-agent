		...
		try:
				...
				self._send_json(
					{
						...
						if enabled
						else 'Automatización pausada. Las búsquedas manuales siguen disponibles.',
					}
				)
				return
			if parsed.path == '/api/batch':
				if not self._automation_enabled():
					self._send_json(
						{'error': 'La automatización está desactivada. Actívala para ejecutar lotes de auto-postulación.'},
						HTTPStatus.CONFLICT,
					)
					return
				...
				if not self.batch_runner.start(request):
					self._send_json(
						{'error': 'Ya hay un lote en ejecución.', 'status': self.batch_runner.status()}, HTTPStatus.CONFLICT
					)
					return
			...
			if parsed.path == '/api/search':
				...
				if not self.collector.start(request):
					self._send_json(
						{'error': 'Ya hay una búsqueda en ejecución.', 'status': self.collector.status()}, HTTPStatus.CONFLICT
					)
					return
			...
			if match:
				if not self._automation_enabled():
					self._send_json(
						{'error': 'La automatización está desactivada. Actívala antes de postular automáticamente.'},
						HTTPStatus.CONFLICT,
					)
					return
				...
				if not self.preparer.start(job_id):
					self._send_json(
						{'error': 'Ya hay una postulación en ejecución.', 'status': self.preparer.status()}, HTTPStatus.CONFLICT
					)
					return
