...
class DashboardHandler(BaseHTTPRequestHandler):
	...
	def do_GET(self) -> None:  # noqa: N802
		...
		if parsed.path in static:
			self._send_static(*static[parsed.path]); return
		...
		if parsed.path == "/api/jobs":
			...
			try: min_score = max(0, min(100, int(query.get("min_score", ["0"])[0])))
				...
				self._send_json({"error": "min_score must be an integer"}, HTTPStatus.BAD_REQUEST); return
		...
		if match:
			self._send_json(self.store.list_application_attempts(int(match.group("job_id")))); return
		self.send_error(HTTPStatus.NOT_FOUND)
		...
		try:
			...
			if parsed.path == "/api/search":
				if self.preparer.status()["state"] == "running":
					self._send_json({"error": "Hay una postulación en ejecución. Espera a que termine antes de buscar."}, HTTPStatus.CONFLICT); return
			...
			if match:
				if self.collector.status()["state"] == "running": self._send_json({"error": "Hay una búsqueda en ejecución. Espera a que termine antes de postular."}, HTTPStatus.CONFLICT); return
