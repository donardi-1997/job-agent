...
class DashboardHandler(BaseHTTPRequestHandler):
	...
	def _read_json(self) -> dict[str, object]:
		...
		if length <= 0: return {}
		if length > 131_072: raise ValueError("Request body too large")
		decoded = json.loads(self.rfile.read(length).decode("utf-8"))
		if not isinstance(decoded, dict): raise ValueError("JSON body must be an object")
		return decoded
	...
	def do_GET(self) -> None:  # noqa: N802
		...
		if parsed.path == "/api/batch/history": self._send_json(self.store.list_batch_runs()); return
		if parsed.path == "/api/jobs":
			query = parse_qs(parsed.query)
			try: min_score = max(0, min(100, int(query.get("min_score", ["0"])[0])))
			except ValueError: self._send_json({"error": "min_score must be an integer"}, HTTPStatus.BAD_REQUEST); return
			status = query.get("status", [None])[0] or None
			self._send_json(self.store.list_jobs(min_score=min_score, status=status)); return
		...
		if match:
			...
			self._send_json(job); return
		match = JOB_DRAFT_RE.match(parsed.path)
		if match:
			draft = self.store.get_application_draft(int(match.group("job_id")))
			if not draft: self._send_json({"error": "Aún no hay respuestas guardadas para esta vacante."}, HTTPStatus.NOT_FOUND); return
			self._send_json(draft); return
		match = JOB_ATTEMPTS_RE.match(parsed.path)
		if match: self._send_json(self.store.list_application_attempts(int(match.group("job_id")))); return
	...
	def do_POST(self) -> None:  # noqa: N802
		...
		try:
			...
			if parsed.path == "/api/batch":
				...
				if not self.batch_runner.start(request): self._send_json({"error": "Ya hay un lote en ejecución.", "status": self.batch_runner.status()}, HTTPStatus.CONFLICT); return
				self._send_json(self.batch_runner.status(), HTTPStatus.ACCEPTED); return
			if parsed.path == "/api/search":
				if self._automation_busy() or self.preparer.status()["state"] == "running":
					self._send_json({"error": "Hay otra operación de navegador en ejecución."}, HTTPStatus.CONFLICT); return
				request = SearchRequest.model_validate(body)
				if not self.collector.start(request): self._send_json({"error": "Ya hay una búsqueda en ejecución.", "status": self.collector.status()}, HTTPStatus.CONFLICT); return
				self._send_json(self.collector.status(), HTTPStatus.ACCEPTED); return
			...
			if match:
				...
				if not job: self._send_json({"error": "Vacante no encontrada."}, HTTPStatus.NOT_FOUND); return
				if job.get("status") == "applied": self._send_json({"error": "Esta vacante ya figura como postulada."}, HTTPStatus.CONFLICT); return
				if not self.preparer.start(job_id): self._send_json({"error": "Ya hay una postulación en ejecución.", "status": self.preparer.status()}, HTTPStatus.CONFLICT); return
				self._send_json(self.preparer.status(), HTTPStatus.ACCEPTED); return
