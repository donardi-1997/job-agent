...
def classify_question(question: str, *, known_locations: Iterable[str] = ()) -> SemanticMatch:
    ...
    if location and any(
        term in normalized
        for term in ("resides", "vives", "domiciliado", "domiciliada", "radicado", "radicada")
    ):
...
def semantic_similarity(left: str, right: str) -> float:
    ...
    if left_match.intent != "unknown" and right_match.intent != "unknown":
        ...
        if (
            ...
            return 0.0
        if (
            left_match.required_cefr
            and right_match.required_cefr
            and left_match.required_cefr != right_match.required_cefr
        ):
            return 0.0
...
class LocalSemanticAnswerEngine:
        ...
        try:
            with sqlite3.connect(self.path) as connection:
                exists = connection.execute(
                    "SELECT 1 FROM sqlite_master WHERE type='table' AND name='cv_profile'"
                ).fetchone()
                if not exists:
                    return ""
    ...
    def resolve_profile(
        ...
        *,
        ...
        if match.intent == "city" and profile.city:
            ...
            if (
                match.required_years is not None
                and match.year_comparison
                and self._looks_yes_no(options_tuple)
            ):
                raw = (
                    "Sí"
            ...
            allowed = (
                match.mode == "remote"
                and (profile.remote_ok or any("remot" in item for item in preferred))
            ) or (
                match.mode == "hybrid"
                and any("hibrid" in item or "hybrid" in item for item in preferred)
            ) or (
                match.mode == "onsite"
                and any("presencial" in item or "onsite" in item for item in preferred)
            )
            if allowed:
                raw = (
                    "Sí"
    ...
    def adapt_to_options(
        ...
        *,
        ...
        normalized_options = [
            (normalize_semantic(option), option)
            for option in options
            if normalize_semantic(option)
        ]
        ...
        if intent in {"total_experience_years", "skill_experience_years"}:
            ...
            if years is not None:
                if (
                    ...
                    expected = (
                        "yes"
                        if _matches_comparison(years, match.required_years, match.year_comparison)
                        else "no"
                    )
                    for _, option in normalized_options:
    ...
    def _compose_safe_text(intent: str, profile: UserProfile, job: dict[str, object]) -> str:
        ...
        verified = [
            ...
            if normalize_semantic(skill) and normalize_semantic(skill) in description
        ] or profile.skills[:5]
        ...
        if intent == "motivation":
            parts = [
                f"Me interesa la oportunidad de {role}{company_fragment} porque está alineada con mi perfil profesional."
            ]
            if skills:
