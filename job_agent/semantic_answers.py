...
def classify_question(question: str, *, known_locations: Iterable[str] = ()) -> SemanticMatch:
    ...
    for candidate in known_locations:
        ...
        if clean and clean in normalized:
            ...
            break
    if not location:
        location_match = re.search(r"\b(?:resides|vives|domiciliado|radicado)\s+en\s+(.+)$", normalized)
        if location_match:
            tail = re.split(r"\b(?:actualmente|hoy|para|o|y)\b", location_match.group(1), maxsplit=1)[0]
            location = " ".join(tail.split()[:5]).strip()
    scores = {}
...
def semantic_similarity(left: str, right: str) -> float:
    ...
    if ma.intent != "unknown" and mb.intent != "unknown":
        ...
        if ma.intent == "residence_yes_no" and ma.location and mb.location and normalize_semantic(ma.location) != normalize_semantic(mb.location):
            return 0.0
